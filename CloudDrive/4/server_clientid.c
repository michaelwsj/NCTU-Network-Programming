...
int main(void){
	...
	while(1){
		...
		if(FD_ISSET(listenfd, &rset)){
			...
			clientid_str = strtok(NULL, " :\n");
			clientid = atoi(clientid_str);

			for(n = 0; n <= maxi; n++){
				if(clientid == cli_id[n]){
					client[n] = connfd;
					break;
				}
			}
		...
		for(int i = 0; i <= maxi; i++){
			...
			if(FD_ISSET(sockfd, &rset)){
				if((n = read(sockfd, recvline, MAXLINE)) == 0){
					...
					client[i] = -1;	// offline for the client
