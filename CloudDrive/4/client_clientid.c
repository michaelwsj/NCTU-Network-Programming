...
int main(int argc, char **argv){
	...
	char *cmd, *clientid, *opcode, *dataport_str;
	FILE *frptr, *fwptr;
	int n;
	...
	while(1){
		...
		else if(sizeof(clientid) != 5){
			fprintf(stderr, "Error: Incorrect number of digits for client ID. Must be five-digit.\n");
		}
		else{
			fprintf(stderr, "Error: You should enter client ID first.\n");
			fprintf(stderr, "Usage: CLIENTID: <client_ID>.\n");
		}
	}
